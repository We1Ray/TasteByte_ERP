...
-- Only inserted if referenced test users already exist (they are created
-- in migration 041). If users don't exist yet, migration 045 will handle
-- these inserts after the users are available.
-- ══════════════════════════════════════════════════════════════════════

DO $$
BEGIN
    -- Guard: only insert sample data if the referenced test users exist
    IF EXISTS (SELECT 1 FROM users WHERE id = 'a0000000-0000-0000-0000-000000000003') THEN
